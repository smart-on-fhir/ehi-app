...
export default {
  ...
  jobsDir: join(
    __dirname,
    env.NODE_ENV === "test" ? "jobs/test-data" : "jobs/data"
  ),

  // How often to check export status (in milliseconds)
  statusCheckInterval: +(env.STATUS_CHECK_INTERVAL || 5000),

  // How long should deleted jobs stick around for (in milliseconds)
  deletedJobLifetime: +(env.DELETED_JOB_LIFETIME || 10000),
